  ...
  return (
    <section id='projects' className="h-screen flex items-center py-16">
      <div className="max-w-6xl mx-auto px-4 w-full">
        <h2 className="text-4xl font-mono font-bold mb-16">My Projects...</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div 
              key={index}
              className="border-2 border-black rounded-xl p-6 hover:shadow-lg transition-all group"
            >
              <h3 className="text-2xl font-mono font-bold mb-4">{project.name}</h3>
              <p className="text-gray-600 mb-6 line-clamp-4">{project.description}</p>
              
              <div className="flex flex-wrap gap-2 mb-6">
                {project.techStack.map((tech, techIndex) => (
                  <span 
                    key={techIndex}
                    className="px-3 py-1 text-sm font-mono bg-gray-100 rounded-full"
                  >
                    {tech}
                  </span>
                ))}
